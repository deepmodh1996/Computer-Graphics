...
class LineSegment
{
  ...
  public:
    ...
    {
      // TODO: The formula is in the paper
      return ((p-pts[0])*(pts[1]-pts[0]))/(this->length2());
    }
    ...
    {
      // TODO: The formula is in the paper
      Vec2 perpendicular( (-1)*(this->direction()).y(), (this->direction()).x());
      return ((p-pts[0])*(perpendicular))/ ( perpendicular.length() );
    }
    ...
    double segmentDistance(Vec2 const & p, double u, double v) const
    {
      ...
      if(u>1){ return (this->end() - p).length();}
      else if(u<0){ return (this->start() - p).length();}
      else{return abs(v);}
    }
